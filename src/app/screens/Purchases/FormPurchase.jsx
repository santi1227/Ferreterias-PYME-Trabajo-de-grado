...
export const FormPurchase = ({ formData, isEdit, handleChange, handleDirectChange }) => {
        ...
        const fetchInitialData = async () => {
            try {
                const [providersRes, productsRes] = await Promise.all([
                    fetch("/api/providers?page=1&limit=100"),
    ...
    useEffect(() => {
        if (formData && Array.isArray(formData.products) && isEdit) {
            ...
            const formattedProvider =
                typeof formData.provider === "object"
            ...
            if (formData.provider !== formattedProvider) {
                handleDirectChange("provider", formattedProvider);
            }
        }
    }, [formData]);
    ...
    const handleTempChange = (field, value) => {
        setTempProduct(prev => {
            ...
            if (field === "product" || field === "quantity" || field === "price") {
        ...
        <Box sx={{ maxWidth: 1000, mx: "auto", mt: 4 }}>
            ...
            <Box sx={{ display: "flex", gap: 2, mt: 3 }}>
                <TextField
                    ...
                    onBlur={(e) => handleTempChange("price", products.find(p => p._id === e.target.value)?.purchasePrice || 0)}
                    fullWidth
                >
